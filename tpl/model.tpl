// auto-generated by terser-cli {{.GenerateTime}}
...
func ({{.VarName}} *{{.StructName}}) Load() error {
	return New{{.StructName}}Query().
		First(&{{.VarName}}{{range .PrimaryKeys}}, {{$.VarName}}.{{.PropertyName}}{{end}}).Error
}

func Find{{.StructName}}ByID({{range .PrimaryKeys}}{{.VarName}} {{.GoDataType}}, {{end}}) ({{.VarName}} *{{.StructName}}, err error) {
	where := map[string]interface{}{
		{{range .PrimaryKeys}}{{$.StructName}}_{{.PropertyName}}: {{.VarName}},
{{end}}{{if .LogicDeleteKey}}
		{{.StructName}}_{{.LogicDeleteKey}}:  0,{{end}}
	}
...
func Delete{{.StructName}}ByID({{range .PrimaryKeys}}{{.VarName}} {{.GoDataType}}, {{end}}{{if .UpdateUserKey}}adminUserID string{{end}}) (err error) {
{{if .LogicDeleteKey}}
	...
	return New{{.StructName}}Query().
		Where("{{range .PrimaryKeys}}{{.Name}} = ?{{end}}"{{range .PrimaryKeys}}, {{.VarName}}{{end}}).
...
func Update{{.StructName}}ByID({{range .PrimaryKeys}}{{.VarName}} {{.GoDataType}}, {{end}}updates interface{}) (err error) {
	return New{{.StructName}}Query().
		Where("{{range .PrimaryKeys}}{{.Name}} = ? {{end}}"{{range .PrimaryKeys}}, {{.VarName}}{{end}}).
